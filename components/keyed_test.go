...
func TestKeyed(t *testing.T) {
	...
	t.Run("keyer compares by key", func(t *testing.T) {
	...
	t.Run("disposes reactive children", func(t *testing.T) {
		...
		assert.Equal(t, 1, children[1].MountCalls(), "removed child should be mounted once")
		assert.Equal(t, 0, children[1].UpdateCalls(), "removed child should not be updated after removal")
		assert.Equal(t, 1, children[1].UnmountCalls(), "removed child should be unmounted once after removal")

		// update a signal used inside a node that was removed. and make sure it doesn't cause an update on the removed node
		setters[1](1)
		assert.Equal(t, 1, children[1].MountCalls(), "removed child should be mounted once")
		assert.Equal(t, 0, children[1].UpdateCalls(), "removed child should not be updated after removal")
		assert.Equal(t, 1, children[1].UnmountCalls(), "removed child should be unmounted once after removal")
